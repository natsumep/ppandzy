...
export const isOutOfChinaExtent = (coordinate) => {
...
export function WGS84ToGCJ02(coordinate) {
  if (isOutOfChinaExtent(coordinate)) {
    return coordinate;
  }
  const a = 6378245.0;
  ...
  let dLat = _transformLatitude(lon - 105.0, lat - 35.0);
  let dLon = _transformLongitude(lon - 105.0, lat - 35.0);
  ...
  const mgLat = lat + dLat; // y
  const mgLon = lon + dLon; // x
  return [mgLon, mgLat];
}
